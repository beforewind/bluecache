...
uint32_t* keylenArray;
uint32_t* vallenArray;
...
void flushDmaBuf(){
  sendEom();
  if ( srcBuf_offset > 0 && currSrcBase != (uint32_t)-1 ){
...
void dmaBufMemreadBuf(void* respBuf_void, size_t totalSize){
  char* respBuf = (char*) respBuf_void;
...
void resetDMADstBuf(){
  //sendEom();
  if ( dstBuf_offset > 0 && currDstBase != (uint32_t)-1 ){
    //device->freeWriteBufId(currDstBase);
...
void initBluecache(){
  ...
  uint32_t numBufs = ALLOC_SZ/DMABUF_SZ;
...
int runtest(int argc, const char ** argv)
{
  ...
  device = new BluecacheRequestProxy(IfcNames_BluecacheRequestS2H);
  ...
  #ifndef BSIM
  std::cout << "Enter number of keylen: ";
  std::cin >> keylen;

  std::cout << "Enter number of vallen: ";
  std::cin >> vallen;
  ...
  keylenArray = new uint32_t[numTests];
  vallenArray = new uint32_t[numTests];
  ...
  for ( int i = 0; i < numTests; i++ ){
      sendSet(keyArray[i], valArray[i], keylenArray[i], vallenArray[i], i);
  }
  flushDmaBuf();
